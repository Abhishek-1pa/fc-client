...
import { Button, Card, CardContent, CardMedia, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, IconButtonProps, Stack, Typography, styled } from '@mui/material';
...
const BlogItem = ({ blog }: { blog: BlogPostData }) => {
  ...
  const handleConfirmDelete = () => {
    ...
    if(blog && blog.id)
      store.dispatch(deleteBlog(blog.id)).then(response=> {
  ...
  return (
    ...
    onClick = {()=>    store.dispatch(setCurrentBlog(blog))}>
      ...
      <h4 style={{
        ...
        overflow:"hidden",
        width:"180px"
        }} onClick = {handleBlogClick} >{blog.title}</h4>
